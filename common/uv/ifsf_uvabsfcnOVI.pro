...
;
;-
function ifsf_uvabsfcnOVI, wave, param, lratio, tratio, modhei=modhei, modnadabs=modnadabs, $
...
;   ilo = 3+nhei*3
   ...
   if nnadabs gt 0 then begin
      ...
      for i=0,nnadabs-1 do begin
         modnadabs[*,i]*=ifsf_cmpuvabsOVI(wave,param[ilo+i*4:ilo+(i+1)*4-1])
         modtmp*=ifsf_cmpuvabsOVI(wave,param[ilo+i*4:ilo+(i+1)*4-1])
      endfor
      modflux *= modtmp
