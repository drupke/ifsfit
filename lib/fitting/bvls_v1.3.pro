...
;
; CALLING SEQUENCE:
;   BVLS, A, B, BND, X, [RNORM, NSETP, W, INDEX, IERR,]
;       EPS=eps, ITMAX=itmax, ITER=iter, /FASTNORM
;
; DESCRIPTION:
;
...
;
;   This IDL version has been ported from the original Fortran 90 code
...
;   V1.3 Added compilation options and converted to IDL V5.0, MC, Leiden 20 May 2002
;-
;----------------------------------------------------------------------
...
IERR = 0
mch = MACHAR()
IF KEYWORD_SET(eps1) THEN EPS = eps1 ELSE EPS = mch.EPS
IF KEYWORD_SET(itmax1) THEN ITMAX = itmax1 ELSE ITMAX = 3L*N
ITER = 0L
;
IZ2 = N
IZ1 = 1L
...
END ; ( INITIALIZE )
;----------------------------------------------------------------------
PRO BVLS, A1, B1, BND1, X1, RNORM1, NSETP1, W1, INDEX1, IERR1, $
EPS=EPS1, ITMAX=ITMAX1, ITER=ITER1, FASTNORM=fastNorm1
